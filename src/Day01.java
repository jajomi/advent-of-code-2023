...
public class Day01 {
  ...
  private static List<String> numbersAsWords() {
  ...
  private static Map<String, String> wordsToNumbers() {
    Map<String, String> numbersAsWordsMap = new HashMap<>();
    numbersAsWordsMap.put("one", "1");
    numbersAsWordsMap.put("two", "2");
    numbersAsWordsMap.put("three", "3");
    numbersAsWordsMap.put("four", "4");
    numbersAsWordsMap.put("five", "5");
    numbersAsWordsMap.put("six", "6");
    numbersAsWordsMap.put("seven", "7");
    numbersAsWordsMap.put("eight", "8");
    numbersAsWordsMap.put("nine", "9");
    return numbersAsWordsMap;
  }


  public static void main(String[] args)  {
//    findCalibrationValue(testCalibrationDocument);
//    findCalibrationValue(testCalibrationDocument2);
    findCalibrationValue(calibrationDocument);
  }

  private static void findCalibrationValue(String[] document) {
    int sumOfCalibrationValues = 0;

    for (String calibrationDocumentLine : document) {
      String firstDigit = findFirstDigit(calibrationDocumentLine);
      String lastDigit = findLastDigit(calibrationDocumentLine);

      int concatenatedDigits = Integer.parseInt(firstDigit + lastDigit);

      sumOfCalibrationValues += concatenatedDigits;

      System.out.println(calibrationDocumentLine);
      System.out.println(String.format("%s + %s = %s", firstDigit, lastDigit, concatenatedDigits));
    }

    System.out.println(String.format("Sum of calibration values: %s", sumOfCalibrationValues));
  }


  public static String findFirstDigit(String inputString) {
    for (int i = 0; i < inputString.length(); i++) {
      if (Character.isDigit(inputString.charAt(i))) {

        return inputString.substring(i, i+1);

      } else if (isNumberAsWord(inputString.substring(i))) {

        return wordsToNumbers().get(retrieveStartingNumberAsWord(inputString.substring(i)));
  ...
  public static String findLastDigit(String inputString) {
    for (int i = inputString.length() - 1; i >= 0; i--) {
      if (Character.isDigit(inputString.charAt(i))) {
        return inputString.substring(i, i+1);
      } else if (isEndingNumberAsWord(inputString.substring(0, i+1))) {
        return wordsToNumbers().get(retrieveEndingNumberAsWord(inputString.substring(0, i+1)));
      }
    }
  ...
  private static boolean isNumberAsWord(String inputString) {
    return retrieveStartingNumberAsWord(inputString) != null;
  }

  private static String retrieveStartingNumberAsWord(String inputString) {
    for (String numberAsWord : numbersAsWords()) {
      if (inputString.startsWith(numberAsWord)) {
        return numberAsWord;
      }
    }
  ...
  private static boolean isEndingNumberAsWord(String inputString) {
    return retrieveEndingNumberAsWord(inputString) != null;
  }

  private static String retrieveEndingNumberAsWord(String inputString) {
    for (String numberAsWord : numbersAsWords()) {
      if (inputString.endsWith(numberAsWord)) {
        return numberAsWord;
      }
    }
  ...
  *     Input data
  */
  final static String[] testCalibrationDocument = new String[] {
      "1abc2",
      "pqr3stu8vwx",
      "a1b2c3d4e5f",
      "treb7uchet"
  ...
  final static String[] testCalibrationDocument2 = new String[] {
      "two1nine",
      "eightwothree",
      "abcone2threexyz",
      "xtwone3four",
  ...
  final static String[] calibrationDocument = new String[] {
